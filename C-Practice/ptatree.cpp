...
#define MaxTree 10
#define Null -1 //区别于系统的NULL 0

typedef int Status; //Status是函数的类型,其值是函数结果状态代码，如OK等
typedef char ElementType; //ElementType类型根据实际情况而定,这里假设为char
...
Status Isomprphic(int root1, int root2){
    ...
    if((T1[root1].left == Null) && (T2[root2].left == Null) && (T1[T1[root1].left].data == T2[T2[root2].left].data))//两树左子树皆不为空,且值相等
        return (Isomprphic(T1[root1].left, T2[root2].left) && Isomprphic(T1[root1].right, T2[root2].right));
    else //两树左子树有一个空,或者,皆不空但值不等
        return (Isomprphic(T1[root1].left, T2[root2].right)&& Isomprphic(T1[root1].right,T2[root2].left));//交换左右子树判断
}
